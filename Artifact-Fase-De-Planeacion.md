...
> [!IMPORTANT]
> **1. Fuente de la Tasa BCV:** ¿Existe un API específico que "Suministros L&D" ya usa para la tasa del día, o debemos integrar una fuente pública (ej. `exchangerate.host`, `pydolarvenezuela`)? Esto impacta la implementación del hook `use-bcv-rate` y la ruta `/api/bcv-rate`.

> [!IMPORTANT]
> **2. Imágenes de Productos:** ¿Se dispone de fotografías reales de los productos del catálogo, o debemos planificar un sistema de placeholder genéricos por categoría? Las imágenes reales son críticas para la confianza B2B.

> [!IMPORTANT]
> **3. Migración de Inter a Geist:** La skill *design-taste-frontend* prohíbe Inter. ¿Aprobamos la migración a Geist Sans como body font, o documentamos Inter como excepción aceptada?

> [!WARNING]
> **4. Cashea Integration:** ¿Cashea ofrece un SDK/API para calcular cuotas dinámicamente, o se usa un cálculo aproximado (monto / 6 meses)? Esto define si el componente `PriceDisplay` necesita integración externa o solo aritmética local.

> [!WARNING]
> **5. Emojis en WhatsApp:** El mensaje actual usa emojis. La skill de diseño los prohíbe en la interfaz web, pero ¿se aprueba también eliminarlos del mensaje de WhatsApp enviado al chat? Algunos usuarios podrían preferir los emojis en WhatsApp por legibilidad rápida en chat.
